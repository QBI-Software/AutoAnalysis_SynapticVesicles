...
class Normalized(AutoData):
    ...
    def setConfigurables(self,cfg):
        ...
        if 'EXPT_NORM' in cfg.keys() and cfg['EXPT_NORM'] is not None:
            self.norm = cfg['EXPT_NORM']
            if self.norm.startswith('*'):
                self.norm = self.norm[1:]
        if 'ROI_FILE' in cfg.keys() and cfg['ROI_FILE'] is not None:
            roifile = cfg['ROI_FILE']
            if roifile.startswith('*'):
                self.roifile = self.datafile.replace(self.suffix,roifile[1:])
            else:
                self.roifile = join(self.outputdir,self.roifile)
        if 'STIM_BELOW' in cfg.keys() and cfg['STIM_BELOW'] is not None:
            self.stimbelow = cfg['STIM_BELOW']
        if 'STIM_ABOVE' in cfg.keys() and cfg['STIM_ABOVE'] is not None:
            self.stimabove = cfg['STIM_ABOVE']
        if 'MAX_BELOW' in cfg.keys() and cfg['MAX_BELOW'] is not None:
            self.maxbelow = cfg['MAX_BELOW']
        if 'MAX_ABOVE' in cfg.keys() and cfg['MAX_ABOVE'] is not None:
            self.maxabove = cfg['MAX_ABOVE']
    ...
    def run(self):
        ...
        if not self.data.empty:
            ...
            roilist = self.loadROIlist(self.roifile)
            df_selected = self.data[roilist]
            # Normalize to baseline
            #Get stimulus index
            stimidx = self.getStimulusIndex()
            #Average 10 frames before stim for each ROI and subtract
            df_norm = df_selected.apply(lambda col: self.subtractAvg(col,stimidx,self.stimbelow,self.stimabove))
            print("Normalized data: \n", df_norm)
            # Find max in normalized data
            df_max = df_norm.apply(lambda col: self.divideMaxAvg(col,self.maxbelow,self.maxabove))
            # Add Average data
            ...
            outputfile = self.datafile.replace(self.suffix,self.norm)
...
def create_parser():
    ...
    parser.add_argument('--roifile', action='store', help='ROI list', default="*ROIlist_selected.csv")
